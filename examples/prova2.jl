...
function (model::ContinuousTransformerModel)(x)
    # x shape: (embedding_dim, n_samples)
    ...
    # Pass through transformer encoder layers (attention across embedding vectors)
    for layer in model.transformer_layers
        x_reshaped = layer(x_reshaped)  # Each layer applies residual connections internally
    end
...
function train_continuous_transformer!(
    ...
    for epoch in 1:epochs
        ...
        # Generate random batch starting positions for this epoch
        n_possible_batches = div(size(train_inputs, 2) - batch_size + 1, 1)
        n_batches_this_epoch = min(n_batches_per_epoch, n_possible_batches)
        
        # Randomly sample batch starting positions without replacement
        batch_starts = randperm(n_possible_batches)[1:n_batches_this_epoch] .* 1 .- 1 .+ 1
        ...
        for batch_start in batch_starts
            ...
            batch_targets = train_targets[:, batch_start:batch_end]
            
            # Forward and backward pass
            loss, grads = Flux.withgradient(model) do m
                predictions = m(batch_inputs)
        ...
        for batch_start in 1:batch_size:size(val_inputs, 2)
            ...
            batch_targets = val_targets[:, batch_start:batch_end]
            
            predictions = model(batch_inputs)
            val_loss += Flux.mse(predictions, batch_targets)
            n_val_batches += 1
        end
